...
def extract_agent_reports(
    ...
    *,
    ...
    Truncation is per-report at `max_chars`. Pass `max_chars=0` to keep
    full bodies (e.g. when storing reports back into the memory archive).
    """
...
def extract_errors(entries: Iterable[dict], limit: int = 5, snippet_len: int = 300) -> list[str]:
    ...
    patterns are dropped — they carry no resume value."""
    errs: list[str] = []
    for e in entries:
        if e.get("type") != "user":
            continue
...
def extract_compact_summaries(entries: Iterable[dict]) -> list[str]:
    ...
    by the report's brief-vs-CC comparison panel."""
    out: list[str] = []
    for e in entries:
